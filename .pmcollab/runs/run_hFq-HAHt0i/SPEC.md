# Change Specification: Protection tests — Balance Territory Coverage (balance-territory-coverage#1)
...
| Field | Value |
|-------|-------|
| Title | Protection tests — Balance Territory Coverage (balance-territory-coverage#1) |
| Target Spec Document | Balance Territory Coverage (current promoted version) |
| Baseline Version | current |
| Baseline Section Range | n/a — test-surface change only |
| Change Type | code_align — characterization tests |
| Source | Codebase Preparation Journey remediation batch `balance-territory-coverage#1` (UC-102 §14.2) |
| Priority | Risk-ranked by the remediation plan |

### Problem / Opportunity

3 criteria of "Balance Territory Coverage" have no proven test protection — nothing
...
|---|---------|------------|---------|---------|
| 1 | 17. Affected Test Surface | MODIFIED | Characterization tests | Add: one characterization test per unprotected criterion (3 row(s)) |
...
#### TC-1 — `bmb_e_O9SGVZ3Vot`

**Behavior under test:** Executive referrals are assigned to Avery Johnson and escalated for VP Sales direct engagement.

- **This attempt (2):** STRENGTHEN the existing test for this behavior — do not add a second one. The protection check deliberately broke the code below and no test failed:
  - _The check did not record which breaks — strengthen the assertions on the outcome this statement asserts._
  Add assertions on the outcome each of these changes, so the test fails when any of them is made.
...
- **Annotation to add:** `@pmc-criterion master:business-rule:executive-referrals-go-to-avery`

#### TC-2 — `bmb_PKO7Pv4DWm07`

**Behavior under test:** Sales operations can review each representative's territory, segment, product focus, configured capacity, and current routing load in one team view. This supports capacity planning, territory oversight, and fair distribution of leads across the sales team.

- **This attempt:** nothing — an earlier attempt already proved this test notices a break. Leave it as it is.

- **Protection today:** proven — Already meets the protection bar; strengthen only if the rows below say so.
- **Annotation to add:** `@pmc-criterion master:use-case:balance-territory-coverage`

#### TC-3 — `bmb_zjwI1dzbe_z2`

**Behavior under test:** A lead is Enterprise at 1,000 or more employees or ARR above $10M, Mid-Market at 100 or more employees or ARR of at least $1M when not Enterprise, and SMB otherwise.

- **This attempt:** nothing — an earlier attempt already proved this test notices a break. Leave it as it is.

- **Protection today:** proven — Already meets the protection bar; strengthen only if the rows below say so.
- **Annotation to add:** `@pmc-criterion master:business-rule:enterprise-thresholds-determine-lead-segment`
...
**Population:** 3 ratified and 0 provisional
...
- TC-1: `leadRouter.js`
- TC-2: `leadRouter.js`, `index.html`, `styles.css`
- TC-3: `leadRouter.js`
