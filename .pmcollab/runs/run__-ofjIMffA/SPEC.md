# Change Specification: Protection tests — Route Leads to Best Rep (route-leads-to-best-rep#1)
...
| Field | Value |
|-------|-------|
| Title | Protection tests — Route Leads to Best Rep (route-leads-to-best-rep#1) |
| Target Spec Document | Route Leads to Best Rep (current promoted version) |
| Baseline Version | current |
| Baseline Section Range | n/a — test-surface change only |
| Change Type | code_align — characterization tests |
| Source | Codebase Preparation Journey remediation batch `route-leads-to-best-rep#1` (UC-102 §14.2) |
| Priority | Risk-ranked by the remediation plan |

### Problem / Opportunity

2 criteria of "Route Leads to Best Rep" have no proven test protection — nothing
...
#### TC-1 — `bmb_aRD9TxRgCruJ`

**Behavior under test:** Sales operations can route inbound leads, partner referrals, and event-sourced opportunities to the most appropriate representative using account, geography, segment, product interest, and source rules. When the preferred rep is unavailable or full, the system can overflow the lead to the next-best rep or SDR pool to keep response times fast and ownership clear.
...
- **Annotation to add:** `@pmc-criterion master:use-case:route-leads-to-best-rep`

#### TC-2 — `bmb_TJKIzmg_E3WC`

**Behavior under test:** Copilot Studio POC requests are assigned to Alex Rivera while he has capacity, otherwise to Casey Kim or Quinn Torres as backups.
...
- **Protection today:** not measured — Not enough evidence from a protection check to judge — treat the existing coverage as unproven.
- **Annotation to add:** `@pmc-criterion master:business-rule:copilot-pocs-prioritize-alex-rivera`
...
- TC-1: `index.html`, `leadRouter.js`, `styles.css`
- TC-2: `leadRouter.js`
